...
指令都要求 **@ 机器人**，都由插件直接回复 —— 不进 dsh、不消耗模型调用；其中会话相关的六条限 `approvers` 名单（不在名单里的人会收到一句拒绝，名单为空则谁都不能用），`/id` 不受任何限制。

| 指令           | 作用                                                                         |
| -------------- | ---------------------------------------------------------------------------- |
| `/current`     | 看当前会话的 ID 和最近使用时间                                               |
...
| `/stop`        | 停掉当前会话正在跑的回合。排队等着的消息会一起丢掉                           |
...
- **回合异常会播报**：回合以 `error` 或撞到输出上限（`max-tokens`）收尾时，插件往那个群/单聊发一句提示；`/stop` 停掉的和正常结束的不播报
