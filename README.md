...
QQ 群消息 ──▶ 归一化 ──▶ 历史库（全部入库）
              ...
              dsh agent 跑一个完整回合（思考、调工具、可以调 bash…）
                    ...
                    └─▶ qqbot_send_file 把磁盘上的文件发出去 ──▶ QQ 群
```
...
```sh
pnpm test    # 152 项：@标记剥离/事件归一化/附件传递/访问控制/时间格式化/入库检索/审批卡片/切分/配置/会话指令/会话绑定/上下文折叠/文件发送
```
...
- **`qqbot_send_file` 的通用文件消息需要平台侧的「文件消息」权限**，没开通时平台会拒绝
  （错误原样返回给 agent，不会静默失败）。图片/视频/语音的大小上限分别是
  30MB / 100MB / 20MB，通用文件 100MB。
