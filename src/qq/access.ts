...
export function decideAccess(
  msg: PeerRef & { senderId: string },
  config: AccessConfig,
): AccessDecision {
  const isGroup = msg.scope === "group";
  const allowed = isGroup ? config.allowedGroups : config.allowedUsers;
  if (!allowed.includes(msg.peerId)) {
    return {
      action: "deny-peer",
      reason: isGroup ? "group-not-allowed" : "c2c-not-allowed",
    };
  }
