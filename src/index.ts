...
export function apply(ctx: Context, config: Config): void {
  ...
  const approval = new ApprovalChannel({
    ...
    send: (target, text, keyboard) =>
      gateway.sendCard(target.scope, target.peerId, text, keyboard),
    findTarget: (sessionId) => sessions.findTarget(sessionId),
    currentSessionOf: (scope, peerId) =>
      store.getCurrentSession(config.appId, scope, peerId)?.sessionId,
    logger,
  });
  ...
  async function buildContextReply(
    ...
    try {
      const summary = summarizeSessionLog(
        (agent as unknown as { session: SessionLogLike }).session,
      );
  ...
  async function handleSessionCommand(
    ...
    if (command.kind === "new") {
      ...
      try {
        ...
        titleSession(agent);
        const binding = store.createSession(
          config.appId,
          scope,
          peerId,
          String(agent.id),
        ...
        logger.info(
          `[dsh-yashiro] 已新建会话：${shortSessionId(String(agent.id))} peer=${peerId}`,
        );
    ...
    logger.info(
      `[dsh-yashiro] 已切换会话：${shortSessionId(target.id)} peer=${peerId}`,
    );
    ...
    async function reply(text: string): Promise<void> {
      try {
        ...
        logger.error(
          `[dsh-yashiro] 发送会话指令回复失败: ${describeError(err)}`,
        );
      }
    }
  }
  ...
  function titleSession(agent: AgentLike): void {
    ...
    try {
      ...
      logger.warn(
        `[dsh-yashiro] 设置会话标题失败（不影响对话）：${describeError(err)}`,
      );
    }
  }
  ...
  async function handleMessage(msg: StoredMessage): Promise<void> {
    ...
    if (isIdCommand(msg.content, msg.mentionsBot)) {
      try {
        ...
        logger.info(
          `[dsh-yashiro] 已响应 ${ID_COMMAND}：${msg.scope} ${msg.peerId}`,
        );
      } catch (err) {
        logger.error(
          `[dsh-yashiro] 回复 ${ID_COMMAND} 失败: ${describeError(err)}`,
        );
      }
      return;
    }
    ...
    if (access.action === "deny-peer") {
      logger.debug(
        `[dsh-yashiro] 会话未放行（${access.reason}）：${msg.scope} ${msg.peerId}`,
      );
      return;
    }
    ...
    if (!msg.mentionsBot) {
      logger.debug(
        `[trace] 非 @ 消息，仅入库: ${JSON.stringify(msg.content.slice(0, 40))}`,
      );
      return;
    }
    ...
    if (current === undefined) {
      logger.info(
        `[dsh-yashiro] 无会话，已提示 ${msg.scope} ${msg.peerId} 用 /new`,
      );
      try {
        ...
        logger.error(
          `[dsh-yashiro] 发送「无会话」提示失败: ${describeError(err)}`,
        );
      }
      return;
    }
    ...
    if (config.announceNewMessageCount && previousWake !== undefined) {
      try {
        newSinceLastWake = store.countSince(
          config.appId,
          msg.peerId,
          previousWake,
          true,
    ...
    try {
      logger.debug(
        `[trace] 准备连到会话 session=${shortSessionId(current.sessionId)} peer=${msg.peerId}`,
      );
      const agent = await sessions.select(
        msg.scope,
        msg.peerId,
      ...
      if (agent === undefined) {
        logger.error(
          `[dsh-yashiro] 会话连不上：${shortSessionId(current.sessionId)}`,
        );
