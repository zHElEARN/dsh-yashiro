...
export function summarizeSessionLog(log: SessionLogLike): ContextSummary {
  ...
  const summary: ContextSummary = {
    ...(typeof log.header?.createdAt === 'number' ? { createdAt: log.header.createdAt } : {}),
    cacheReadTokens: 0,
    running: false,
    turns: 0,
    steps: 0,
  ...
  for (const event of events) {
    switch (event.type) {
      ...
      case 'request/context': {
        ...
        if (typeof context?.contextWindow === 'number') summary.contextWindow = context.contextWindow
        break
      }
...
function applyUsage(summary: ContextSummary, usage: UsageLike): void {
  ...
  if (typeof usage.totalTokens === 'number') summary.totalTokens = usage.totalTokens
  summary.cacheReadTokens = cached
  // 累计值缺失时用最后一次的分解兜底，免得那一行空着
  if (summary.totalTokens === undefined) summary.totalTokens = input + cached + output
}

function contextModel(context: RequestContextLike | null | undefined): string | undefined {
