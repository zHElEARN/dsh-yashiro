...
4. **@ 了机器人**：投递到「当前会话」的 agent 回合 —— agent 跑一个完整回合（思考、调工具、
   可以调 bash），期间用三个工具跟 QQ 侧打交道：
...
**架构图、设计取舍表只写在文档里**，代码里不再复述 —— 两处都写必然会漂移。

### 不要重复同一份知识

sessionKey 的格式、出站消息怎么落库这类事实，全仓库只能有一处定义
（现在分别是 `sessionKeyOf()` 和 `store.appendOutbound()`）；第二处出现就该抽出来。
同理，配置项的含义写在 README 的「配置项」表里、平台坑写在本文档里，代码注释只指路不复述。
...
4. **`ctx.agents.create()` 不会自己查默认模型**。`dsh-agent-default-model` 的定位是
...
> 群主必须在群设置里给机器人开「接收所有消息」，否则非 @ 消息根本不会推过来 ——
> 这一条属于使用侧，写在 README 的「使用提示与限制」里。
...
`pnpm test` 会先跑 biome（`pnpm lint`）、再 typecheck、再 build，最后跑 `dist/` 上的用例 ——
所以它同时能抓到「改了 src 忘了重新构建」和格式/import 顺序跑偏这类问题。
...
- **会话标题 = 8 位短 ID**：`/new` 和 `/switch` 时都会用短 ID 给会话命名，这样它在
  **web 侧边栏**里认得出是哪条（不设的话标题会退回工作目录名，所有会话长得一样）。
  ...
  会在 agent 第一次回话时被顶掉。
- **`/context` 的上下文占用**取最近一次请求的 `inputTokens + cacheReadTokens`（含缓存
  命中部分），这才是「上下文现在有多大」。**不要**用 `usage.totalTokens` —— 它是跨请求
  累计值，多轮之后会远大于窗口，看起来像超额了。还没跑过回合的会话不编数字，
  缓存命中率不足 100% 时也不四舍五入。
...
- **附件只把元信息交给 agent**：插件不下载、不持久化 —— 由 agent 自己决定何时 `curl`
  下来再 `read_image`。
