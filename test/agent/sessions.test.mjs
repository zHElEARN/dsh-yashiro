...
describe("sessionKeyOf", () => {
  it("形如 yashiro:appId:scope:peerId:epoch", () => {
    assert.equal(
      sessionKeyOf(APP, "group", "G1", 1),
      `yashiro:${APP}:group:G1:1`,
    );
  ...
  it("scope 参与区分：同一个 id 在群聊和单聊是两条会话", () => {
    assert.notEqual(
      sessionKeyOf(APP, "group", "X", 1),
      sessionKeyOf(APP, "c2c", "X", 1),
    );
  });

  it("不同 appId 不串会话", () => {
    assert.notEqual(
      sessionKeyOf("a", "group", "G1", 1),
      sessionKeyOf("b", "group", "G1", 1),
    );
  });
